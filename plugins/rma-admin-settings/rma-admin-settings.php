...
final class RMA_Admin_Settings {
    ...
    public function render_newsletter_page(): void {
        ...
        echo '<div class="wrap"><h1>Lista Newsletter</h1>';
        echo '<p>Lista automática de todos os usuários cadastrados no site.</p>';
        echo '<p><a class="button button-primary" href="' . esc_url($csv_url) . '">Exportar CSV</a> <a class="button" href="' . esc_url($xml_url) . '">Exportar XML</a></p>';
    ...
    public function render_reports_page(): void {
        ...
        echo '<div class="wrap"><h1>Relatórios</h1>';
        echo '<p>Relatórios detalhados de faturamento, entidades ativas/inativas e situação de governança.</p>';
        $scope_links = [
            'all' => 'Todos',
        ...
        foreach ($scope_links as $scope_key => $scope_label) {
            ...
            echo '<a style="margin-right:6px" class="' . esc_attr($class) . '" href="' . esc_url($url) . '">' . esc_html($scope_label) . '</a>';
        }
        echo '</p>';
        echo '<p><a class="button button-primary" href="' . esc_url($csv_url) . '">Exportar Relatório CSV</a> <a class="button" href="' . esc_url($xml_url) . '">Exportar Relatório XML</a></p>';
        echo '<table class="widefat striped" style="max-width:900px"><tbody>';
        echo '<tr><th>Total de entidades</th><td>' . esc_html((string) ($summary['total_entities'] ?? 0)) . '</td></tr>';
        echo '<tr><th>Entidades ativas (adimplentes)</th><td>' . esc_html((string) ($summary['active'] ?? 0)) . '</td></tr>';
        echo '<tr><th>Entidades inativas/inadimplentes</th><td>' . esc_html((string) ($summary['inactive'] ?? 0)) . '</td></tr>';
        echo '<tr><th>Governança aprovada</th><td>' . esc_html((string) ($summary['governance_approved'] ?? 0)) . '</td></tr>';
        echo '<tr><th>Governança pendente/recusada</th><td>' . esc_html((string) ($summary['governance_pending'] ?? 0)) . '</td></tr>';
        echo '<tr><th>Anuidades com vencimento em até 30 dias</th><td>' . esc_html((string) ($summary['due_30'] ?? 0)) . '</td></tr>';
        echo '<tr><th>Faturamento (pedidos pagos WooCommerce)</th><td>R$ ' . esc_html(number_format((float) ($summary['revenue'] ?? 0), 2, ',', '.')) . '</td></tr>';
        echo '</tbody></table>';

        echo '<h2 style="margin-top:24px">Detalhamento das entidades (' . esc_html(strtoupper($scope)) . ')</h2>';
    ...
    public function render_subscription_emails_page(): void {
        ...
        echo '<div class="wrap"><h1>E-mails Assinatura</h1>';
        echo '<p>Edite os e-mails programados da assinatura anual.</p>';
        ...
        foreach ($events as $key => $label) {
            ...
            echo '<h2 style="margin-top:24px">' . esc_html($label) . '</h2>';
            echo '<p><label>Assunto<br/><input class="regular-text" type="text" name="subject_' . esc_attr($key) . '" value="' . esc_attr($subject) . '" /></label></p>';
            echo '<p><label>Corpo<br/><textarea name="body_' . esc_attr($key) . '" rows="6" class="large-text">' . esc_textarea($body) . '</textarea></label></p>';
        }
