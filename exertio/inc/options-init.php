		...
		<style>
			...
			(function(){
				...
				const mapSvg = root.querySelector('.box-mapa svg#map, .box-mapa svg');
					...
					let mapDimensions = { width: 460, height: 465 };
					...
					function detectMapDimensions(){
						...
						if (Number.isFinite(attrWidth) && attrWidth > 0 && Number.isFinite(attrHeight) && attrHeight > 0) {
							mapDimensions = { width: attrWidth, height: attrHeight };
						}
					}
				...
				function mapCoordsFromGeoOrState(marker){
					const lat = Number(marker && marker.latitude);
					const lng = Number(marker && marker.longitude);
					if (Number.isFinite(lat) && Number.isFinite(lng)) {
						...
						const x = ((lng - minLng) / (maxLng - minLng)) * mapDimensions.width;
							...
							if (Number.isFinite(x) && Number.isFinite(y) && x >= 5 && x <= (mapDimensions.width - 5) && y >= 5 && y <= (mapDimensions.height - 5)) {
							return [x, y];
						}
					}
					const state = String((marker && marker.state) || '').toLowerCase();
						...
						return [mapStatePoints[state][0] * scaleX, mapStatePoints[state][1] * scaleY];
				}
