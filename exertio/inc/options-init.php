...
if (!function_exists('rma_map_get_adimplente_map_markers')) {
	function rma_map_get_adimplente_map_markers($limit = 250)
	{
		...
		foreach ($ids as $entity_id) {
			...
			$state = strtolower((string) rma_map_meta_value($entity_id, array('uf', '_employer_state', '_state')));
...
"
		...
		if (!empty($entity_ids)) {
			foreach ($entity_ids as $entity_id) {
				...
				$entity_state = strtolower((string) rma_map_meta_value($entity_id, array('uf', '_employer_state', '_state')));
			...
			(function(){
				...
				function renderMapPins(markers){
					...
					markers.forEach((marker, idx) => {
						...
						title.textContent = `${marker.name || ''}${marker.city ? ' - ' + marker.city : ''}/${String(marker.state || '').toUpperCase()}`;
