...
"
		...
		if (!empty($entity_ids)) {
			foreach ($entity_ids as $entity_id) {
				...
				if (!rma_map_search_matches_entity($search, array($name, $nome_fantasia, $razao_social, $search_slug, $search_cnpj, $entity_city, $address))) {
					continue;
				}
			...
			<div class="rma-map-directory" data-endpoint="<?php echo esc_url($endpoint); ?>" data-per-page="<?php echo esc_attr($per_page); ?>">
				...
				async function load(page = 1){
					...
					for (const [k,v] of fd.entries()) {
						...
						const textLimit = k === 'search' ? 120 : (k === 'city' ? 80 : 100);
