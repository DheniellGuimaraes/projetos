...
if (!function_exists('rma_map_directory_shortcode')) {
	function rma_map_directory_shortcode($atts = array())
	{
		...
		<style>
			...
			#map{display:block;max-width:760px;width:100%;height:auto}
			#map .state{cursor:pointer;transition:fill .18s ease,stroke .18s ease,opacity .18s ease}
			#map .brazil-outline{fill:rgba(74,222,128,.12);stroke:#166534;stroke-width:1.3}
			#map .state .shape{fill:#86efac;stroke:#4d7c0f;stroke-width:1}
			#map .state .label_icon_state{fill:#14532d;font-family:Arial,sans-serif;font-size:11px;font-weight:700}
				#map .state:hover,#map .state.is-active{fill:#22c55e !important;stroke:#166534 !important;opacity:1}
			...
			(function(){
				...
				function getStateAnchorFromSvg(state){
					...
					const stateNode = mapSvg.querySelector('#state_' + state + ', .state[data-state="' + state + '"]');
					if (!stateNode) return null;
					const shapeNode = stateNode.querySelector('.shape, path, polygon, circle, ellipse, rect') || stateNode;
					if (!shapeNode || typeof shapeNode.getBBox !== 'function') return null;
					try {
				...
				function renderMapPins(markers){
					...
					const oldGroup = mapSvg.querySelector('#rma-map-pins');
					...
					markers.forEach((marker, idx) => {
						...
						pin.setAttribute('d', `M ${x} ${y-10} C ${x-4} ${y-10}, ${x-7} ${y-7}, ${x-7} ${y-3} C ${x-7} ${y+3}, ${x} ${y+9}, ${x} ${y+9} C ${x} ${y+9}, ${x+7} ${y+3}, ${x+7} ${y-3} C ${x+7} ${y-7}, ${x+4} ${y-10}, ${x} ${y-10} Z`);
						...
						title.textContent = markerLocation ? `${markerName} - ${markerLocation}` : markerName;
