...
if (!function_exists('rma_map_directory_shortcode')) {
	function rma_map_directory_shortcode($atts = array())
	{
		...
		<style>
			...
			#map .brazil-outline{fill:rgba(134,239,172,.18);stroke:#4d7c0f;stroke-width:1.5}
			...
			#map .model-davi .state:hover .shape,#map .model-davi .state.is-active .shape{fill:#dc2626 !important}
			...
			(function(){
				...
				function mapCoordsFromGeoOrState(marker){
					...
					if (Number.isFinite(lat) && Number.isFinite(lng)) {
						...
						const x = ((lng - minLng) / (maxLng - minLng)) * 460;
						const y = ((maxLat - lat) / (maxLat - minLat)) * 465;
						if (Number.isFinite(x) && Number.isFinite(y) && x >= 10 && x <= 450 && y >= 10 && y <= 455) {
							return [x, y];
						}
					}
					const state = String((marker && marker.state) || '').toLowerCase();
					return mapStatePoints[state] || null;
				}
